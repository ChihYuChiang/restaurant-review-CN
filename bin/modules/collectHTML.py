...
def mainPage(shopId, outputPath, **kwargs):
    ...
    #Retry connection a numbmer (3) of times for each request
    for attempt in range(3):
        try:
            ...
            if attempt + 1 == RETRY:
                print(r'currentItem = {}'.format(currentItem))
                raise
    ...
    try:
        ...
        WebDriverWait(browser, 5).until(expected_conditions.presence_of_element_located((By.CSS_SELECTOR, '.brief-info > a.icon')))
    ...
    try:
        #Wait until certain elements loaded
        WebDriverWait(browser, 5).until(expected_conditions.presence_of_element_located((By.CSS_SELECTOR, '.recommend-name + .J-more')))
